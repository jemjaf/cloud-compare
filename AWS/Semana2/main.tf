resource "aws_eip" "database_eip" {
  ...
  associate_with_private_ip = aws_instance.database_instance.private_ip
...
output "ssh_connect_db" {
  value = "ssh -i ec2_instance_key_pair.pem ubuntu@${aws_instance.database_instance.private_ip}"
}