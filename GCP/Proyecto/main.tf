...
resource "google_compute_firewall" "app_fw_ingress" {
  name        = "${local.project_name}-app-fw-ingress-${local.environment}"
  network     = google_compute_network.rafita_vpc.name
  description = "App security group - Ingress - ${local.environment}"
  direction   = "INGRESS"
...
resource "google_compute_firewall" "db_fw_ingress" {
  name        = "${local.project_name}-db-fw-ingress-${local.environment}"
  network     = google_compute_network.rafita_vpc.name
  description = "Database security group - Ingress - ${local.environment}"
  direction   = "INGRESS"
